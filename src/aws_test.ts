...
class MockSTSServer {
    ...
    setResponse(params: { action: string; roleArn: string; token: string }, response: string) {
...
async function createTempTokenFile(content: string): Promise<string> {
    ...
    const tempFile = await Deno.makeTempFile({ dir: "/tmp/deno", suffix: ".token" });
...
Deno.test("webIdentityTokenProvider - successful credential exchange", async () => {
    ...
    try {
        ...
        assertEquals(credentials.secretAccessKey, "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY");
        assertEquals(credentials.sessionToken, "AQoEXAMPLEH4aoAH0gNCAPyJxz4BlCFFxWNE1OPTgk5TthT+FvwqnKwRcOIfrRh3c/LTo6UDdyJwOOvEVPvLVXEi...");
...
Deno.test("webIdentityTokenProvider - token file does not exist", async () => {
    ...
    Deno.env.set("AWS_WEB_IDENTITY_TOKEN_FILE", "/tmp/deno/non-existent-token-file");
...
Deno.test("webIdentityTokenProvider - validates request parameters", async () => {
    ...
    globalThis.fetch = async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
        ...
        return new Response(createSuccessfulSTSResponse(
            "AKIAIOSFODNN7EXAMPLE",
            "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
            "sessionToken",
            "2023-11-03T14:30:00Z"
    ...
    try {
        ...
        assertEquals(capturedUrl?.searchParams.get("Action"), "AssumeRoleWithWebIdentity");
        assertEquals(capturedUrl?.searchParams.get("RoleArn"), roleArn);
        assertEquals(capturedUrl?.searchParams.get("RoleSessionName"), "deno-web-identity-session");
        assertEquals(capturedUrl?.searchParams.get("WebIdentityToken"), tokenContent);
